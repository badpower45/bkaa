...
BEGIN
    ...
    IF v_draft.barcode IS NOT NULL AND TRIM(v_draft.barcode) <> '' THEN
        SELECT id INTO v_existing_id FROM products 
        WHERE barcode::text = TRIM(v_draft.barcode) LIMIT 1;
    END IF;

    IF v_existing_id IS NULL AND v_draft.name IS NOT NULL THEN
        SELECT id INTO v_existing_id FROM products 
        WHERE LOWER(TRIM(name)) = LOWER(TRIM(v_draft.name)) LIMIT 1;
    END IF;
    ...
    ELSE
        ...
        INSERT INTO products (
            ...
            COALESCE(
                v_product_id_candidate, 
                (SELECT COALESCE(MAX(CAST(id AS INTEGER)), 0) + 1 FROM products WHERE id ~ '^[0-9]+$')::TEXT
            ),
            v_draft.name,
...
-- 3. تأكيد النجاح
SELECT '✅ تم تحديث الـ function بنجاح! التصنيفات هتشتغل دلوقتي.' AS status;
